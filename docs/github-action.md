...
`resolve`), `cross-reviewer-dedup` (default `true`), `github-token`. Each maps to a `LOUPE_*` env var (see below); config/prompt
...
`LOUPE_PRIOR_COMMENTS`, `LOUPE_CROSS_REVIEWER_DEDUP`.

...
6. **Parse.** The last `{...}` in stdout is the review. It must carry a string `summary`, so `{}` or `{"status":"done"}` is a failure, not a clean review; `findings` and `concerns` may be omitted when empty but must be arrays when present. Each finding is validated on its own; malformed entries are dropped and counted. Off-scale severities (`critical`, `minor`, ...) map onto blocker, warning, nit.

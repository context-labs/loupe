...
4. **Prompts.** Every in-scope PR file's patch is rendered as a `### <path>` section and written to a temp file. The user prompt carries the file tree, that path, the cwd-to-repo path mapping when `dir` is set, and on an incremental run the list of files to reassess. See [What the agent sees](./context.md).
